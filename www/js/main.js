...
require([
  ...
  "luminous.min",			//画像用モーダルウィンドウJS
	...
	//スマートフォン用ボタン 閉じるボタン
	var globalNaviSmallIconCloseButton = document.querySelector('#globalNaviSmallIcon--closeButton');
	globalNaviSmallIconCloseButton.addEventListener( 'click' , btn_is_close );
	
	function btn_is_close(){
		if( globalNaviSmallIcon.classList.contains("is-open") == true ){
			globalNaviSmallIcon.classList.remove("is-open");
			MicroModal.close('modal-globalNaviSmall', {
				awaitCloseAnimation: true
...
require([
  ...
  "gsap.min",				//アニメーションJS
	...
	function getDuration() {
		return $('#main').height() - $('#aside__inner').innerHeight();
	}
